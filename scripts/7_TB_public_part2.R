...
# gene_set_list <- list(c("IFITM1","CD274","TAP1","GBP5","GBP2","FCGR1CP"))
gene_set_list <- list(c("IFITM1","TAP1","GBP5","GBP2","FCGR1CP"))
...
this.accession.res.dir <- file.path(output.dir, this.accession.no, "4_genes")
...
validation_genelists_func <- function(numberofgenes){
  
this.accession.res.dir <- file.path(output.dir, this.accession.no, paste0(numberofgenes,"_genes"))
if(!exists(this.accession.res.dir)) dir.create(this.accession.res.dir)
...
for (pair in pairwise_comparisons) {
  ...
  # GSVA
  gene_set_list_hgnc <- list(c("IFITM1","CD274","TAP1","GBP5","GBP2","S100A8","FCGR1CP"))
  
  if(numberofgenes == "6"){
  gene_set_list_hgnc <- list(c("IFITM1","CD274","TAP1","GBP5","GBP2","FCGR1CP"))}
  
  if(numberofgenes == "5"){
  gene_set_list_hgnc <- list(c("IFITM1","TAP1","GBP5","GBP2","FCGR1CP"))}
  
  if(numberofgenes == "4"){
  gene_set_list_hgnc <- list(c("TAP1","GBP5","GBP2","FCGR1CP"))}
  
  signature_geneid <- as.character(gene_annot[match(gene_set_list_hgnc[[1]], gene_annot$Symbol), "GeneID"])
  gene_set_list <- list(c(signature_geneid))
...
disease_roc <- ggplot(disease_roc_data, aes(x = FPR, y = TPR, color = legend)) +
  ...
  labs(
    ...
    y = "TPR (Sensitivity)",
    color = "Comparison",
    caption = paste0("Signature: ", paste0(gene_set_list_hgnc[[1]], collapse = ","))) 
...
validation_genelists_func(numberofgenes = "6")
validation_genelists_func(numberofgenes = "5")
validation_genelists_func(numberofgenes = "4")
...
for (pair in pairwise_comparisons) {
  ...
  signature_geneid <- as.character(gene_annot[match(gene_set_list_hgnc[[1]], gene_annot$Symbol), "ID"])
  gene_set_list <- list(c(signature_geneid))
...
for (pair in pairwise_comparisons) {
  ...
  gene_set_list_hgnc <- list(c("IFITM1","CD274","TAP1","GBP5","GBP2","S100A8","FCGR1B"))
  
  if(numberofgenes == "6"){
    gene_set_list_hgnc <- list(c("IFITM1","CD274","TAP1","GBP5","GBP2","FCGR1B"))}
  
  if(numberofgenes == "5"){
    gene_set_list_hgnc <- list(c("IFITM1","TAP1","GBP5","GBP2","FCGR1B"))}
  
  if(numberofgenes == "4"){
    gene_set_list_hgnc <- list(c("TAP1","GBP5","GBP2","FCGR1B"))}
  
  signature_geneid <- as.character(gene_annot[match(gene_set_list_hgnc[[1]], gene_annot$GENE_SYMBOL), "REFSEQ"])
  gene_set_list <- list(c(signature_geneid))
...
clinical_heatmap_ordered <- clinical_heatmap[order(clinical_heatmap$Disease),] #only T0
...
# clinical_heatmap_ordered <- clinical_heatmap[order(clinical_heatmap$Disease),] #only T0
clinical_heatmap_ordered <- clinical_heatmap[order(clinical_heatmap$condition),]
...
ggsave(heatmapplot, filename = file.path(this.figure.dir, "heatmap_alltimepoints_genesig_D_7.png"), 
       width = 3200, 
       height = 2800, 
       units = "px")
