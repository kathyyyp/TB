...
for (i in 1:nrow(mean_sig_zscore)){
  glm_data <- data.frame(Score = mean_sig_zscore[i,], Group = clinical_auc$Disease)
  ...
  res <-cbind(
    signature = row.names(mean_sig_zscore)[i],
    auc = auc(roc_obj),
  ...
  disease_roc_objects[[row.names(mean_sig_zscore)[i]]] <- roc_obj
  
  
    # Sensitivity confidence interval
  ci_sens <- ci.se(roc_obj, specificities =  as.numeric(optimal_threshold_coords["sensitivity"]))
  
  # Specificity confidence interval
  ci_spec <- ci.sp(roc_obj, sensitivities =  as.numeric(optimal_threshold_coords["specificity"]))
...
res_table[,-1] <- lapply(res_table[,-1], as.numeric)
  ...
  auc_forest_plot <-  res_table %>% 
    ...
    ylab("AUC")+
    xlab("Signature")+
    labs(title = "Performance of published signatures",
    caption = paste0("Forest plot showing AUCs for differentiating TB_T0 vs HC_T0 \nin our dataset\nSensitivity and specificity calculated at Youden threshold\n95% confidence interval shown"))

  
  
  
  ggsave(auc_forest_plot, filename= file.path(this.forest.dir, "mean_score_forestplot.png"),
...
write.csv(timepoint_full_tableofres, file.path(this.roc.dir,"timepoint_other_published_signatures_tableofauc.csv"))
