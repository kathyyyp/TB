...
# Method used to generate signature scores: Mean of Z-scores
# ================================================================================== #
# A. SCRIPT SET UP =================================================================
# ================================================================================== #
...
#For 3. mean_zscore
current.gsva.dir <- file.path(current.roc.dir, "mean_zscore", "ML_16")
...
# ================================================================================== #
# 3. MEAN OF Z SCORES USING the 16 GENES ORDERED BY PERMUTATION IMPORTANCE  ====================
#    TO DECIDE ON SIZE OF GENE SET
# ================================================================================== #
# Mean of z-scored gene expression with cross-validation using 16-gene signature as ordered by above 5-fold CV with RF model 
gene_sets <- list()
...
if(!exists(file.path(current.roc.dir, "mean_zscore"))) dir.create(file.path(current.roc.dir, "mean_zscore"))
...
note <- paste0(
  ...
  str_c(top_features_cv, collapse = " "),
  "\nNow calculate mean z-scores with 16 sets")
...
for (set_name in names(gene_sets)) {
  ...
  #Open repeats loop
  results <- foreach(i = 1:repeats, .packages = c("caret", "pROC", "stats", "base")) %dopar% {
    ...
    for (fold in seq_along(cv_folds)) {
      ...
      train_data <- expression_auc[train_indices, ] #genes are columns, samples are rows
      test_data <- expression_auc[test_indices, ]
      
      
      # Get the scores for training set ----------------------------------------------------------------------------------
      #Subset for our x number of genes 
      train_gene_set <- train_data[,selected_features]  #genes are columns so we can z-score column-wise (centre = centre each gene around its own mean)
      ...
      train_gene_set_zscore<-scale(train_gene_set, center=T, scale=T)   # This results in a standardized dataset with mean = 0 and standard deviation = 1 (z-score transformation).
      ...
      train_mean_sig_zscore<- rowMeans(train_gene_set_zscore)

      

      #Get scores for test set-----------------------------------------------------------------------------------------
      #Subset for our x number of genes 
      test_gene_set <- test_data[,selected_features] 
      test_gene_set_zscore<-scale(test_gene_set, center=T, scale=T)   # This results in a standardized dataset with mean = 0 and standard deviation = 1 (z-score transformation).
      #take the mean of the scaled+centred data for each gene and use this as each sample's score
      test_mean_sig_zscore<- rowMeans(test_gene_set_zscore)
      
      
      # Prepare data for GLM
      glm_train_data <- data.frame(Score = train_mean_sig_zscore, Group = train_data$Disease)
      glm_test_data  <- data.frame(Score = test_mean_sig_zscore, Group = test_data$Disease)
...
# ================================================================================== #
# 5. MEAN-ZSCORE WITH VARIOUS SIGNATURE SETS =========================================
# ================================================================================== #

# MEAN 5X5 CV using 6/7/8 gene signatureS ------------------------------------------------------------#

library(plyr)
library(dplyr)
library("doParallel"); library("foreach")

signature_set.dir <- file.path(current.roc.dir, "mean_zscore", "different_gene_sets")
  ...
  for (set_name in names(gene_sets)) {
    ...
    results <- foreach(i = 1:repeats, .packages = c("caret", "pROC", "stats", "base")) %dopar% {
      ...
      for (fold in seq_along(cv_folds)) {
        ...
        glm_train_data <- data.frame(Score = train_mean_sig_zscore, Group = train_data[,type])
        
        # The GLM model we make will be used to predict Timepoint based on the gsva scores in the test set
        glm_test_data  <- data.frame(Score = test_mean_sig_zscore, Group = test_data[,type]) 
...
gene_set_list <- c("IFITM1","CD274","TAP1","GBP5","GBP2","S100A8","FCGR1CP")
...
boxplot_mean_zscore <- as.data.frame(cbind(mean_zscore = mean_sig_zscore,
...
boxplot_mean_zscore$mean_zscore <- as.numeric(boxplot_mean_zscore$mean_zscore)
boxplot_mean_zscore$group <- factor(boxplot_mean_zscore$group)

# View(pivot_wider(boxplot_mean_zscore, names_from = group, values_from = PID))
...
stat.table <- map_dfr(my_paired_comparisons, function(groups) {
  ...
  df <- get_paired_data(data = boxplot_mean_zscore, 
                        group1 = g1, 
                        group2 = g2)
  ...
  if (n_distinct(df$PID) > 0) {
    wilcox_test(df, mean_zscore ~ group, paired = TRUE) %>%
...
stat.table$xmin = c(1,3,3,3)
stat.table$xmax = c(2,4,5,6)

# ================================================================================== #
##  UNPAIRED COMPARISONS  ===================================================================
# ================================================================================== #
stat.table2 <- wilcox_test(boxplot_mean_zscore, mean_zscore ~ group, 
                                paired = FALSE,
...
boxplotfinal <- ggplot(boxplot_mean_zscore, aes(
  ...
  labs(title = paste0("Signature Analysis"),
       caption = paste("Signature: IFITM1, CD274, TAP1, GBP5, GBP2, S100A8, FCGR1CP\n",
                       ...
                       "Signature Score: Mean of z-scored normalised expression of the 7 genes")) +
...
write.csv(boxplot_mean_zscore, file.path(genesig_D_7_performance.figures.dir, paste0("genesig_D_7_", outcome, "_boxplot_gsva_paired_boxplot_data.csv")))
...
# ================================================================================== #
genesig_D_7.dir <- file.path(output.dir, "2.1_genesig_D_7", "mean_zscore")
...
expression_sig4 <- expression_T0[genesig_D_4,]
expression_sig7 <- expression_T0[genesig_D_7,]
...
for (score in c("sig7_zscore", "sig4_zscore")){
  ...
  ggsave(boxplotfinal2, file = file.path(clinical_correlation.dir, "disease", paste0("sex_mean_zscore",score,".png")),  width = 2000,
         height = 2000,
         units = "px" )
...
ggsave(boxplotfinal2, file = file.path(clinical_correlation.dir, "disease", paste0("smoking","_mean_zscore_",score,".png")),  
       width = 2000,
       height = 2000,
       units = "px" )
}
...
ggsave(boxplotfinal_withindisease, file = file.path(clinical_correlation.dir, "disease", paste0("age","withindisease_mean_zscore_",score,".png")),
       width = 2500,
       height = 2000,
       units = "px" )
...
ggsave(boxplotfinal_withinbetweendisease, file = file.path(clinical_correlation.dir, "disease", paste0("age","withinbetweendisease_mean_zscore+",score,".png")),  
       width = 2500,
       height = 2000,
       units = "px" )
...
ggsave(boxplot_withinage, file = file.path(clinical_correlation.dir, "disease", paste0("age","withinage_mean_zscore_",score,".png")),  
       width = 2500,
       height = 2000,
       units = "px" )
...
boxplotfinal2 <- ggplot(boxplot2, aes(
  ...
  ggsave(boxplotfinal2, file = file.path(clinical_correlation.dir, "timepoint", paste0("sex_mean_zscore",score,".png")),  
         width = 2000,
       height = 2000,
       units = "px" )
}
...
ggsave(boxplotfinal2, 
       file = file.path(clinical_correlation.dir, "timepoint", paste0("smoking_mean_zscore_",score,".png")),  
       width = 2800,
       height = 2500,
       units = "px" )
...
ggsave(boxplot_withintimepoint, file = file.path(clinical_correlation.dir, "timepoint", paste0("age_withintimepoint_mean_zscore_",score,".png")),  
       width = 3000,
       height = 2000,
       units = "px" )
...
ggsave(boxplot_withinage, file = file.path(clinical_correlation.dir, "timepoint", paste0("age_withinage_mean_zscore_",score,".png")),  
       width = 3200,
       height = 2300,
       units = "px" )
}
