...
signature.dir <- file.path(output.dir, "signature_generation")
...
if(!exists(file.path(current_roc_dir, "permutation_importance"))) dir.create(file.path(current_roc_dir, "permutation_importance"), recursive = TRUE)

current_importance_dir <- file.path(current_roc_dir, "permutation_importance", "train_16")
...
TB_rna_clnical$pid  %in% clinical$PID #ALL 113 FALSE
...
diffexp_TBT0_vs_HCT0 <- read.csv(file.path(main.dir, "output", "results", "tT2", "contrast1.csv"))
...
write.csv(feature_stats_cv, file = file.path(current_importance_dir, "summarised_feature_rankings.csv"))
...
signature_set.dir <- file.path(current_roc_dir, "GSVA", "different_gene_sets")
...
gene_sets <- list(genesig_A_6, genesig_B_6, 
                  genesig_D_6, genesig_D_7, genesig_D_8, genesig_D_4,
                  genesig_E_6, genesig_E_7, genesig_E_8,
                  genesig_F_6, genesig_F_7, genesig_F_8,
                  )


names(gene_sets) <- c("genesig_A_6", "genesig_B_6", "genesig_D_6", "genesig_D_7", "genesig_D_8", "genesig_D_4",
...
roc_func <- function(type, case, control){
  ...
  if(outcome == "Cured"){
    ...
    clinical_cured <- clinical[which(clinical$treatment_outcome %in% c("Cured") | clinical$Disease == "HC"),]
  ...
  for (set_name in names(gene_sets)) {
    ...
    write.csv(all_results,file = file.path(signature_set.dir, set_name, paste0(set_name, outcome,"_",case,"vs",control,"_all_results.csv")))
    ...
    cat(sprintf("Gene set: %s took %s (hh:mm:ss)\n", set_name, formatted_time))
  } # close loop of the different gene signatures
  
  
  # Save summarised results for each gene signature
  write.csv(nested_cv_results, file = file.path(signature_set.dir, paste0(outcome, "_",case,"vs",control,"_nested_cv_results_different_signatures.csv")))
...
colnames(combined_summary_results)[3:ncol(combined_summary_results)] <- c("TB_T0vsHC_T0",
                                                                          "TB_T2vsTB_T0",
                                                                          "TB_T4vsTB_T0",
                                                                          "TB_T6vsTB_T0",
                                                                          "HC_T6vsHC_T0")

write.csv(combined_summary_results, file = file.path(signature_set.dir, outcome, "all_different_signatures_auc_results.csv"))
} #close outcome loop
...
for (i in files){
  nested_res <- read.csv(file.path(signature_set.dir, 
                                  paste0(outcome, "_", i, "_nested_cv_results_all_gene_sets.csv"))) ##### fix up file read in
...
if(!exists(file.path(signature_set.dir, "boxplot"))) dir.create(file.path(signature_set.dir, "boxplot"))
  ggsave(boxplot, filename = file.path(signature_set.dir, "boxplot", paste0(outcome, "_different_signatures_" ,i, "boxplot.png")),
         width = 3500,
         height = 3200,
         units = "px" )
...
write.csv(nested_res_long, file.path(signature_set.dir, "boxplot_data", paste0(outcome, "_different_signatures_" ,i, "boxplot_data.csv")))
...
genesig_D_7_TBT0vsHCT0 <- readRDS(file.path(signature_set.dir, paste0("genesig_D_7_",  "TB_T0vsHC_T0", "_all_results.RDS"))) ##change to csv
genesig_D_7_TBT2vsTBT0 <- readRDS(file.path(signature_set.dir, paste0("genesig_D_7_",  "TB_T2vsTB_T0", "_all_results.RDS")))
genesig_D_7_TBT4vsTBT0 <- readRDS(file.path(signature_set.dir, paste0("genesig_D_7_",  "TB_T4vsTB_T0", "_all_results.RDS")))
genesig_D_7_TBT6vsTBT0 <- readRDS(file.path(signature_set.dir, paste0("genesig_D_7_",  "TB_T6vsTB_T0", "_all_results.RDS")))
genesig_D_7_HCT6vsHCT0 <- readRDS(file.path(signature_set.dir, paste0("genesig_D_7_",  "HC_T6vsHC_T0", "_all_results.RDS")))
...
this.figure.dir <- "C:/Users/165861_admin/OneDrive - UTS/Documents/RBMB/TB/output_v2/genesig_D_7/figures"
...
# ================================================================================== #
# 8. CORRELATE CLINICAL FEATURES WITH GENESIG_D_4 ==================================
# ================================================================================== #
